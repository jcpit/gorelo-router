      ...
      try {
        await runBusy(button,"Rechecking…",async()=>{
          ...
          if (preparation&&auditObject(preparation.data)) {
            ...
            prepGrid.append(eventDetail("Preparation",preparation.preflightError?"Blocked: "+preparation.preflightError:"Ready")); prepSection.append(prepGrid); if (preparation.preflightError) prepSection.append(node("p",preparation.preflightError==="entity_resolution_failed"?"Gorelo could not resolve one or more client, contact, technician, device, or location values. Correct the alias or association and recheck before processing.":"The Gorelo action could not be prepared. Review the extracted values and rule mappings before processing.","event-error"));
