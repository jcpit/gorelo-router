...
<body>
  ...
  <script nonce="__CSP_NONCE__">
    ...
    async function captureInboundWebhook(source,button) { try { await runBusy(button,"Arming…",async()=>{ await api("/api/v1/inbound-webhook-sources/"+encodeURIComponent(source.id)+"/capture",{method:"POST"}); }); showToast("Capture armed — send one webhook now"); const deadline=Date.now()+15*60*1000; const poll=async()=>{ if(Date.now()>deadline)return; const data=await api("/api/v1/inbound-webhook-sources/"+encodeURIComponent(source.id)+"/capture"); if(data&&data.captured&&typeof data.payload==="string"){ const pre=node("pre",data.payload,"capture-preview"); pre.style.maxHeight="420px"; pre.style.overflow="auto"; const dialog=document.createElement("dialog"); dialog.append(pre); document.body.append(dialog); dialog.showModal(); dialog.addEventListener("click",()=>dialog.close(),{once:true}); return; } setTimeout(poll,2000); }; poll().catch(error=>showToast(error.message,"error")); } catch(error) { showToast(error.message,"error"); } }
