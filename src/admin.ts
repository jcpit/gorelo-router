  ...
  <script nonce="__CSP_NONCE__">
    ...
    async function openWebhookAuditBuilder(event,button) {
      ...
      try {
        ...
        if (!response.ok) { const raw=await response.text(); let data={}; try { data=JSON.parse(raw); } catch {} throw new Error(formatApiError(response,data,raw)); }
        const payload=JSON.parse(await response.text()); const pointers=capturePointers(payload).slice(0,50); const dialog=document.createElement("dialog"); const title=node("h3","Webhook received — build mappings"); const pre=node("pre",JSON.stringify(payload,null,2),"capture-preview"); pre.style.maxHeight="360px"; pre.style.overflow="auto"; const info=node("p","Detected "+pointers.length+" scalar fields. Save these paths to the source mapping editor, then rename variables and save the source."); const save=node("button","Load fields into source editor","btn btn-primary primary small"); save.type="button"; save.onclick=()=>{ dialog.close(); const source=inboundWebhookSources.find((item)=>item.id===event.ingress?.sourceId); if (!source) { showToast("Refresh Setup before editing this source","error"); return; } openInboundWebhookSourceForm(source,button); byId("inboundWebhookMappings").value=pointers.map((pointer,index)=>"field"+(index+1)+" = "+pointer).join("\n"); }; const close=node("button","Close","btn small"); close.type="button"; close.onclick=()=>dialog.close(); dialog.append(title,info,pre,save,close); document.body.append(dialog); dialog.showModal();
