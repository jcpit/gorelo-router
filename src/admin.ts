  ...
  <script nonce="__CSP_NONCE__">
    ...
    async function openWebhookAuditBuilder(event,button) {
      ...
      try {
        ...
        if (!response.ok) { const raw=await response.text(); let data={}; try { data=JSON.parse(raw); } catch {} throw new Error(formatApiError(response,data,raw)); }
        const payload=JSON.parse(await response.text()); const pointers=capturePointers(payload).slice(0,50); const dialog=document.createElement("dialog"); const title=node("h3","Webhook received — build mappings"); const pre=node("pre",JSON.stringify(payload,null,2),"capture-preview"); pre.style.maxHeight="360px"; pre.style.overflow="auto"; const info=node("p","Detected "+pointers.length+" scalar fields. Save these paths to the source mapping editor, then rename variables and save the source."); const save=node("button","Load fields into source editor","btn btn-primary primary small"); save.type="button"; save.onclick=async()=>{ const sourceId=event.ingress?.sourceId; if(!sourceId){showToast("Webhook source is unavailable","error");return;} try { if(!inboundWebhookSources.length) { const data=await api("/api/v1/inbound-webhook-sources"); inboundWebhookSources=data.sources||[]; } const source=inboundWebhookSources.find((item)=>item.id===sourceId); if(!source) throw new Error("Webhook source is unavailable"); dialog.close(); openInboundWebhookSourceForm(source,button); byId("inboundWebhookMappings").value=pointers.map((pointer,index)=>"field"+(index+1)+" = "+pointer).join("\n"); } catch(error) { showToast(error.message,"error"); } }; const close=node("button","Close","btn small"); close.type="button"; close.onclick=()=>dialog.close(); dialog.append(title,info,pre,save,close); document.body.append(dialog); dialog.showModal();
